...
class NeuralNetwork():
    ...
    def init_network(self, neuron_data):
        ...
        for  layer in range(self.network_length-1):
            ...
            for nodes in range(neuron_data[layer]):
                nested_list.append([abs(neural_math.randomNormalValue(0,0.2)) for nodes_next_layer in range(neuron_data[layer+1])])
    ...
    def forward_pass(self, inputs):
        self.nodes[0] = inputs[:-len(self.nodes[-1])]
        ...
        return [output - target for output, target in zip(self.nodes[-1],[ inputs[-1] ])]
    ...
    def backward_pass(self, target ):
        ...
        for current_node in range(len(self.nodes[-1])):
            self.errors[-1][current_node] = neural_math.sigmoid_derivative(self.nodes[-1][current_node])*(target[-1] - self.nodes[-1][current_node])
